...
const ExchangeRates = () => {
  ...
  fetch(`https://api.fxratesapi.com/currencies`)
    ...
    .then((cur) => {
      ...
      const currencyOptions = document.getElementById(
        "currency-options"
      ...
      const addCurrencyButton = document.getElementById(
        "add-currency-button"
      ) as HTMLDivElement;
      const addCurrencySelect = document.getElementById(
        "add-currency-select"
          ...
          if (prevCurrencies) {
            !isAlreadyAdded &&
              localStorage.setItem(
                "currencies",
                prevCurrencies + "," + selectedCurrency
              );
            ...
            !isAlreadyAdded &&
              localStorage.setItem(
                "currencies",
                "EUR,USD,GEL,GBP" + "," + selectedCurrency
              );
            !isAlreadyAdded && addCurrencyToRates(selectedCurrency);
          }
  ...
  const addRateToList = (base: string, currency: string, rate: number) => {
    ...
    getChange(base, (data) => {
      ...
      const changeRate =
        ((today[currency] - yesterday[currency]) / yesterday[currency]) * 100;

      const roundedChangeRate = +changeRate;
      const formattedChangeRate =
        roundedChangeRate >= 0
          ? `+${roundedChangeRate.toFixed(7)}%`
          : `${roundedChangeRate.toFixed(7)}%`;
